...
const u=new URL('/',baseUrl); u.searchParams.set('t227012','1'); u.searchParams.set('cb',process.env.GITHUB_SHA ?? Date.now().toString())
...
try{
 ...
 }catch(error){result.rendererEvaluateTimedOut=true;result.navigationError=result.navigationError??String(error)}
 await mkdir('artifacts',{recursive:true}); await writeFile('artifacts/T227-012-orders-parent.json',JSON.stringify(result,null,2))
 try{await page.screenshot({path:'artifacts/T227-012-orders-parent.png',fullPage:false,timeout:1500})}catch{}
