...
const result = { path, url: baseUrl + path, stage: 'starting', markers: { effect: null }, readyState: null, navigationError: null, consoleErrors: [], pageErrors: [], failedRequests: [], bodyText: '', operationTimeout: null }
...
try {
  ...
  while (Date.now() < deadline) {
    try {
      const s = await withTimeout(page.evaluate(() => ({ bodyText: document.body?.innerText ?? '', effect: document.querySelector('[data-t227007-effect]')?.getAttribute('data-t227007-effect') ?? null, readyState: document.readyState })), 1200, 'dom-snapshot')
      result.bodyText = s.bodyText; result.markers.effect = s.effect; result.readyState = s.readyState
