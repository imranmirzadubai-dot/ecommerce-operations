...
const u=new URL('/',baseUrl); u.searchParams.set('t227013','1'); u.searchParams.set('cb',process.env.GITHUB_SHA ?? Date.now().toString())
...
try{
 ...
 try{const snapshot=await Promise.race([page.evaluate(()=>({bodyText:document.body?.innerText??'',readyState:document.readyState})),new Promise((_,reject)=>setTimeout(()=>reject(new Error('renderer snapshot timeout')),2500))]);result.bodyText=snapshot.bodyText;result.readyState=snapshot.readyState}catch(error){result.rendererEvaluateTimedOut=true;result.navigationError=result.navigationError??String(error)}
 await mkdir('artifacts',{recursive:true}); await writeFile('artifacts/T227-013-orders-div-parent.json',JSON.stringify(result,null,2))
 try{await page.screenshot({path:'artifacts/T227-013-orders-div-parent.png',fullPage:false,timeout:1500})}catch{}
