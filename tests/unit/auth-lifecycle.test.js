...
test('auth bootstrap has one owner', () => {
  ...
  assert.match(context, /restoreSession\(op\.signal\)/)
...
test('sign-out clears the refresh lifecycle', () => {
  ...
  assert.match(context, /await terminateSession\(op\.signal\)/)
})

test('auth operations use generation and abort coordination', () => {
  assert.match(context, /generationRef/)
  assert.match(context, /AbortController/)
  assert.match(context, /controllerRef\.current\?\.abort\(\)/)
  assert.match(context, /generationRef\.current \+= 1/)
})
