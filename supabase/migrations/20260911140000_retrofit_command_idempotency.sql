...
-- Retrofit idempotency into state-changing core commands.

-- Remove the pre-idempotency signatures so Postgres cannot leave ambiguous overloads.
...
declare
  ...
  v_claim record; v_hash text; v_result jsonb;
begin
  ...
  select * into v_claim from public.claim_command_idempotency('create_order',p_idempotency_key,v_hash);
  if not v_claim.is_new then
    return query select (v_claim.result->>'order_id')::uuid,v_claim.result->>'order_number',(v_claim.result->>'customer_id')::uuid;
    return;
  end if;
  ...
  if v_customer_id is null then
    begin
      ...
      values(btrim(p_customer_name),btrim(p_phone),v_normalized_phone,nullif(btrim(coalesce(p_address,'')),''),nullif(btrim(coalesce(p_city,'')),'')) returning id into v_customer_id;
  ...
  perform public.complete_command_idempotency('create_order',p_idempotency_key,v_result);
...
language plpgsql security definer set search_path=pg_catalog, public
as $$
declare v_state text; v_order_number text; v_claim record; v_hash text; v_result jsonb;
begin
  ...
  select * into v_claim from public.claim_command_idempotency('confirm_order',p_idempotency_key,v_hash);
  if not v_claim.is_new then
    return query select (v_claim.result->>'order_id')::uuid,v_claim.result->>'order_number',v_claim.result->>'lifecycle_state'; return;
  end if;
  ...
  perform public.complete_command_idempotency('confirm_order',p_idempotency_key,v_result);
...
begin
  ...
  if btrim(coalesce(p_idempotency_key,''))='' then raise exception using errcode='22023',message='Idempotency key is required'; end if;
  v_hash:=md5(jsonb_build_object('order_id',p_order_id)::text);
  select * into v_claim from public.claim_command_idempotency('cancel_order',p_idempotency_key,v_hash);
  if not v_claim.is_new then
    return query select (v_claim.result->>'order_id')::uuid,v_claim.result->>'order_number',v_claim.result->>'lifecycle_state'; return;
  end if;
  select lifecycle_state,order_number into v_state,v_order_number from public.orders where id=p_order_id for update;
  ...
  perform public.complete_command_idempotency('cancel_order',p_idempotency_key,v_result);
...
-- The technical idempotency ledger is an internal implementation detail.
revoke select on public.command_idempotency from authenticated;
