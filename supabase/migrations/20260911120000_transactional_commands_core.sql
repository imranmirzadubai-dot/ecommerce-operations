...
returns table (customer_id uuid, customer_code text, name text, phone text, normalized_phone text, address text, city text)
...
begin
  ...
  if btrim(coalesce(p_phone,''))='' then raise exception using errcode='22023', message='Phone is required'; end if;
  return query select c.id,c.customer_code,c.name,c.phone,c.normalized_phone,c.address,c.city from public.customers c where c.normalized_phone=regexp_replace(p_phone,'[^0-9+]','','g');
end; $$;
...
create or replace function public.create_order(p_customer_name text,p_phone text,p_address text,p_city text,p_original_amount numeric(12,2),p_items jsonb,p_notes text default null)
returns table(order_id uuid,order_number text,customer_id uuid)
language plpgsql security definer set search_path=pg_catalog, public
as $$
declare v_customer_id uuid; v_order_id uuid; v_order_number text; v_normalized_phone text; v_item jsonb; v_line_no integer:=0; v_description text; v_quantity integer;
begin
...
language plpgsql security definer set search_path=pg_catalog, public
as $$
declare v_state text; v_order_number text;
begin
  if auth.uid() is null or public.app_role() not in ('sales','operations','admin') then raise exception using errcode='42501',message='Authenticated operational role required'; end if;
