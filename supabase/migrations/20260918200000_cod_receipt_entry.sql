...
-- P11-T171: establish the authoritative COD receipt entry command.
-- The command records one parcel-level receipt atomically, validates its
-- relationship to the order-level obligation, and preserves receipt history.
-- Expected-amount snapshot sourcing is hardened separately by P11-T172.

create or replace function public.record_cod_receipt(
  p_cod_obligation_id uuid,
  p_parcel_id uuid,
  p_expected_amount_snapshot numeric(12,2),
...
begin
  ...
  if p_expected_amount_snapshot is null or p_expected_amount_snapshot < 0 or p_expected_amount_snapshot <> round(p_expected_amount_snapshot, 2) then
    raise exception using errcode='22023', message='Expected amount snapshot must be non-negative with at most two decimal places';
  end if;
  ...
  v_claim := public.claim_command_idempotency(
    'record_cod_receipt',
    p_idempotency_key,
    md5(concat_ws('|', p_cod_obligation_id::text, p_parcel_id::text, p_expected_amount_snapshot::text, p_received_amount::text))
  );
  ...
  if found then
    if v_receipt.cod_obligation_id = p_cod_obligation_id
       and v_receipt.expected_amount_snapshot = p_expected_amount_snapshot
...
revoke all on function public.record_cod_receipt(uuid,uuid,numeric,numeric,text) from public;
revoke all on function public.record_cod_receipt(uuid,uuid,numeric,numeric,text) from anon;
grant execute on function public.record_cod_receipt(uuid,uuid,numeric,numeric,text) to authenticated;
