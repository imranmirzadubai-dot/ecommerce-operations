...
-- T058 hardening: keep the allocation ceiling trigger present and enforce it
-- on the canonical parcel_items table. The trigger is deferred so a single
-- transaction can atomically move/rebalance allocations before commit.
...
for each row execute function public.enforce_parcel_item_allocation_quantity();
