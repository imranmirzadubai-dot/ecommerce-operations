-- P10-T152: authoritative NDR -> RTO lifecycle transition.
-- Extends record_delivery_outcome so a parcel in NDR may transition to RTO.
-- NDR remains restricted to Delivered or RTO; other terminal outcomes remain In Transit-only.
...
begin
  ...
  if v_state='NDR' and btrim(p_outcome) not in ('Delivered','RTO') then
    raise exception using errcode='P0001', message='NDR parcels may only transition to Delivered or RTO through this command';
  end if;
  ...
  values(
    ...
    jsonb_build_object('parcel_number',v_parcel_number,'previous_state',v_previous_state,'state',btrim(p_outcome),'outcome_id',v_outcome_id,'occurred_at',v_occurred_at)
  );
  ...
  v_result:=jsonb_build_object('parcel_id',p_parcel_id,'parcel_number',v_parcel_number,'order_id',v_order_id,'previous_state',v_previous_state,'state',btrim(p_outcome),'outcome_id',v_outcome_id,'outcome',btrim(p_outcome),'note',nullif(btrim(coalesce(p_note,'')),''),'occurred_at',v_occurred_at);

  perform public.complete_command_idempotency('record_delivery_outcome',btrim(p_idempotency_key),v_result);

  return query
    select p_parcel_id,v_parcel_number,v_order_id,v_previous_state,btrim(p_outcome),v_outcome_id,btrim(p_outcome),nullif(btrim(coalesce(p_note,'')),''),v_occurred_at;
end;
$$;
