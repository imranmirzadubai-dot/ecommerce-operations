...
select ok((select count(*) = 31 from supabase_migrations.schema_migrations), 'rebuild applied the complete repository migration chain');
...
select ok((select count(*) = 9 from (select p.proname, pg_get_function_identity_arguments(p.oid) args from pg_proc p join pg_namespace n on n.oid=p.pronamespace where n.nspname='public' and p.prosecdef and p.proconfig @> array['search_path=pg_catalog, public'] and p.proname in ('app_role','create_order','confirm_order','cancel_order','cancel_parcel','claim_command_idempotency','complete_command_idempotency','resolve_customer_by_phone','create_profile')) approved), 'all 9 approved SECURITY DEFINER application functions pin search_path');
