...
select ok((select count(*) = 8 from pg_proc p join pg_namespace n on n.oid=p.pronamespace where n.nspname='public' and p.prosecdef and p.proconfig @> array['search_path=pg_catalog, public']), 'all 8 approved SECURITY DEFINER application functions pin search_path');
