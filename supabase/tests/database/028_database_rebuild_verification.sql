-- P3-T077: verify the staging schema is reproducible from the repository migration chain.
-- This is a structural verification; a fresh Supabase branch rebuild is an environment
-- operation and is not performed by this test.

begin;
select plan(8);

select ok((select count(*) >= 34 from supabase_migrations.schema_migrations), 'staging has the expected migration chain applied');
...
select ok((select count(*) = 8 from pg_proc p join pg_namespace n on n.oid=p.pronamespace where n.nspname='public' and p.prosecdef and p.proconfig @> array['search_path=pg_catalog, public'] and pg_get_function_identity_arguments(p.oid) in ('','p_order_id uuid, p_idempotency_key text','p_parcel_id uuid, p_idempotency_key text','p_command_name text, p_idempotency_key text, p_request_hash text','p_command_name text, p_idempotency_key text, p_result jsonb','p_customer_name text, p_phone text, p_address text, p_city text, p_original_amount numeric, p_items jsonb, p_notes text, p_idempotency_key text','p_phone text')), 'all approved SECURITY DEFINER application functions pin search_path');
