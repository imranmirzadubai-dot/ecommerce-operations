...
select ok((select pg_get_functiondef(p.oid) like '%Parcel allocation exceeds ordered quantity%' from pg_proc p join pg_namespace n on n.oid=p.pronamespace where n.nspname='public' and p.proname='allocate_parcel_item'),'single allocation exposes ordered-quantity overage failure');
select ok((select pg_get_functiondef(p.oid) like '%Split allocation exceeds ordered quantity%' or pg_get_functiondef(p.oid) like '%Split allocation exceeds remaining quantity%' from pg_proc p join pg_namespace n on n.oid=p.pronamespace where n.nspname='public' and p.proname='allocate_parcel_items'),'split allocation exposes ordered-quantity overage failure');
