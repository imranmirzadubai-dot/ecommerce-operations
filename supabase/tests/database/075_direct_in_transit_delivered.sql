...
select ok((select position('Only In Transit parcels can receive a delivery outcome' in pg_get_functiondef(p.oid)) > 0 from pg_proc p join pg_namespace n on n.oid=p.pronamespace where n.nspname='public' and p.proname='record_delivery_outcome'),'In Transit is the authoritative precondition for direct delivery');
