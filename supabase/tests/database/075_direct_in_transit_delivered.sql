...
select ok((select pg_get_functiondef(p.oid) like '%Only In Transit parcels can receive a delivery outcome%' and pg_get_functiondef(p.oid) like '%v_state%' and pg_get_functiondef(p.oid) like '%p_outcome%' from pg_proc p join pg_namespace n on n.oid=p.pronamespace where n.nspname='public' and p.proname='record_delivery_outcome'),'In Transit is the authoritative precondition for direct delivery');
