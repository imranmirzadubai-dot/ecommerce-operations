...
select ok((select pg_get_functiondef(p.oid) like '%v_state<>'In Transit'%' and pg_get_functiondef(p.oid) like '%state=btrim(p_outcome)%' from pg_proc p join pg_namespace n on n.oid=p.pronamespace where n.nspname='public' and p.proname='record_delivery_outcome'),'In Transit is the authoritative precondition for direct delivery');
...
select ok((select pg_get_functiondef(p.oid) like '%v_state:=v_state%' and pg_get_functiondef(p.oid) like '%v_previous_state%' from pg_proc p join pg_namespace n on n.oid=p.pronamespace where n.nspname='public' and p.proname='record_delivery_outcome'),'direct delivery preserves transition context');
