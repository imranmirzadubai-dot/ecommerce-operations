...
select ok((select pg_get_functiondef(p.oid) like '%auth.uid() is null or public.app_role() is null%' from pg_proc p join pg_namespace n on n.oid=p.pronamespace where n.nspname='public' and p.proname='cancel_parcel'),'command requires an authenticated application role');
