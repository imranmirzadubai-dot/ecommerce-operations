...
select ok((select strpos(lower(pg_get_functiondef(p.oid)),'auth.uid() is null')>0 and strpos(lower(pg_get_functiondef(p.oid)),'public.app_role() is null')>0 from pg_proc p join pg_namespace n on n.oid=p.pronamespace where n.nspname='public' and p.proname='cancel_parcel'),'command requires an authenticated application role');
