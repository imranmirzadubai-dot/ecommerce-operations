...
select ok((select position('v_received - cr.expected_amount_snapshot' in pg_get_functiondef(p.oid)) > 0
  from pg_proc p
  join pg_namespace n on n.oid = p.pronamespace
  where n.nspname = 'public'
    and p.proname = 'get_cod_financial_reconciliation'),
  'receipt variance is derived from received amount against immutable expected snapshots');
