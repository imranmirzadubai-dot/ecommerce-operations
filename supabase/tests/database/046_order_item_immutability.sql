...
select ok((select pg_get_functiondef(p.oid) like '%v_state <> ''Draft''%' from pg_proc p where p.pronamespace='public'::regnamespace and p.proname='prevent_confirmed_order_item_change'), 'guard applies to every non-Draft lifecycle state');
select ok((select pg_get_functiondef(p.oid) like '%tg_op = ''DELETE''%' from pg_proc p where p.pronamespace='public'::regnamespace and p.proname='prevent_confirmed_order_item_change'), 'guard covers item deletion');
select ok((select pg_get_functiondef(p.oid) like '%tg_op = ''DELETE'' then old else new end%' from pg_proc p where p.pronamespace='public'::regnamespace and p.proname='prevent_confirmed_order_item_change'), 'guard preserves trigger row semantics');
...
select ok((select pg_get_functiondef(p.oid) like '%select lifecycle_state into v_state from public.orders%' from pg_proc p where p.pronamespace='public'::regnamespace and p.proname='prevent_confirmed_order_item_change'), 'guard derives lock state from the owning order');
