...
select ok((select pg_get_triggerdef(t.oid) like '%BEFORE%' and pg_get_triggerdef(t.oid) like '%INSERT%' and pg_get_triggerdef(t.oid) like '%UPDATE%' and pg_get_triggerdef(t.oid) like '%DELETE%' and pg_get_triggerdef(t.oid) like '%order_items%' from pg_trigger t where t.tgname='trg_order_items_immutable_after_confirmation' and t.tgrelid='public.order_items'::regclass), 'guard covers insert, update and delete');
