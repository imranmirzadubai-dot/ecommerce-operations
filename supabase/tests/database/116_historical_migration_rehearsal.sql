...
select ok((select position('parcel' in lower(pg_get_functiondef(p.oid))) = 0 from pg_proc p join pg_namespace n on n.oid=p.pronamespace where n.nspname='public' and p.proname='import_historical_batch'),'rehearsal verifies historical import does not create parcel allocation');
