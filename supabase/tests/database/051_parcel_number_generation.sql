...
select ok((select column_default like '%nextval(%parcel_number_seq%' from information_schema.columns where table_schema='public' and table_name='parcels' and column_name='parcel_number'), 'parcels have an authoritative sequence-backed parcel_number default');
...
select ok((select count(*) = 1 from pg_index i join pg_class c on c.oid=i.indrelid join pg_attribute a on a.attrelid=c.oid and a.attnum=any(i.indkey) where c.relname='parcels' and a.attname='parcel_number' and i.indisunique), 'parcel_number has a unique constraint/index');
