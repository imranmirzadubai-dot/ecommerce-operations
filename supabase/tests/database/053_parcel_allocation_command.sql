...
select ok((select p.prosecdef and p.proconfig @> array['search_path=pg_catalog, public'] from pg_proc p join pg_namespace n on n.oid=p.pronamespace where n.nspname='public' and p.proname='allocate_parcel_item'),'command pins search_path');
...
select ok((select pg_get_functiondef(p.oid) like '%Parcel allocation is permitted only while parcel is Prepared%' from pg_proc p join pg_namespace n on n.oid=p.pronamespace where n.nspname='public' and p.proname='allocate_parcel_item'),'non-Prepared parcels are rejected');
