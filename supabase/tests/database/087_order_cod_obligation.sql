...
select ok((select pg_get_functiondef(p.oid) like '%claim_command_idempotency(''create_cod_obligation''%' and pg_get_functiondef(p.oid) like '%complete_command_idempotency(''create_cod_obligation''%' from pg_proc p join pg_namespace n on n.oid=p.pronamespace where n.nspname='public' and p.proname='create_cod_obligation'),'COD obligation creation is idempotent and retry safe');
...
select ok((select pg_get_constraintdef(c.oid) = 'UNIQUE (order_id)' from pg_constraint c join pg_class cl on cl.oid=c.conrelid join pg_namespace cn on cn.oid=cl.relnamespace where cn.nspname='public' and cl.relname='cod_obligations' and c.contype='u'),'COD obligation remains one-per-order through the authoritative unique constraint');
