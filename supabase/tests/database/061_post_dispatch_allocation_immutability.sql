...
-- pre-dispatch correction paths are restricted to Prepared parcels.
...
select ok((select pg_get_functiondef(p.oid) like '%Split allocation is permitted only while parcel is Prepared%' from pg_proc p join pg_namespace n on n.oid=p.pronamespace where n.nspname='public' and p.proname='allocate_parcel_items'),'split allocation is restricted to Prepared parcels');
...
select ok((select pg_get_functiondef(p.oid) not like '%if v_parcel_state in (''Delivered'',''RTO'',''Lost'',''Damaged'',''Cancelled'')%' from pg_proc p join pg_namespace n on n.oid=p.pronamespace where n.nspname='public' and p.proname='allocate_parcel_item'),'single allocation does not rely on a terminal-state allowlist');
