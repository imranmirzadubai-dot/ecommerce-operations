...
select ok((select position('invalid phone number:' in lower(pg_get_functiondef(p.oid))) > 0 and position('normalized_data' in lower(pg_get_functiondef(p.oid))) > 0 and position('raw source values remain immutable' in lower(pg_get_functiondef(p.oid))) > 0 from pg_proc p join pg_namespace n on n.oid = p.pronamespace where n.nspname = 'public' and p.proname = 'normalize_import_phone_fields'),'invalid phone values become row errors while mapped normalized data is retained and raw source values remain immutable');
