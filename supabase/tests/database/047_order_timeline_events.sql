...
select ok((select pg_get_functiondef(p.oid) like '%order events are immutable%' from pg_proc p join pg_namespace n on n.oid=p.pronamespace where n.nspname='public' and p.proname='prevent_order_event_mutation'), 'timeline event protection is implemented in database');
