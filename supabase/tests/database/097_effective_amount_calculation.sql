...
select ok((select pg_get_function_result(p.oid)='numeric(12,2)' from pg_proc p join pg_namespace n on n.oid=p.pronamespace where n.nspname='public' and p.proname='get_effective_order_amount'),'effective amount is returned at NUMERIC(12,2) precision');
