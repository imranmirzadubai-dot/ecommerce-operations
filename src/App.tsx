...
import { WorkspaceIsolation } from './components/WorkspaceIsolation'
  ...
  return (
    <div className="operations-app">
      <header className="topbar">
        <div className="brand"><span className="brand-mark">EO</span><div><strong>E-Commerce Operations</strong><span>Operations workspace</span></div></div>
        <div className="environment"><span className="status-dot" /><span>{environmentLabel}</span>{authenticated && <button className="signout" onClick={handleSignOut}>Sign out</button>}</div>
      </header>
      <div className="app-body">
        <aside className="sidebar" aria-label="Primary navigation">
          <nav>{navigation.map((item, index) => <button className={index === 0 ? 'nav-item active' : 'nav-item'} disabled={!authenticated} key={item} onClick={() => navigateTo(item)}><span>{item}</span>{index === 0 && <span className="nav-badge">Core</span>}</button>)}{canAdministerUsers(auth.profile) && <button className="nav-item" onClick={() => document.getElementById('admin-users-title')?.scrollIntoView({ behavior: 'smooth' })}><span>Admin Users</span><span className="nav-badge">Admin</span></button>}</nav>
