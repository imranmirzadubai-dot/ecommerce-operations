...
function App() {
  ...
  function addItem() {
    setItems((current) => [...current, { description: '', quantity: '1' }])
  }
  ...
  async function handleCreateOrder(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      const result = await createOrder(auth.accessToken, {
        p_customer_name: customerName.trim(),
        p_phone: phone.trim(),
        p_address: address.trim() || null,
        p_city: city.trim() || null,
        p_original_amount: Math.round(numericAmount * 100) / 100,
        p_items: cleanItems,
  ...
  return (
    <div className="operations-app">
      <header className="topbar">
        ...
        <div className="environment"><span className="status-dot" /><span>STAGING</span>{authenticated && <button className="signout" onClick={handleSignOut}>Sign out</button>}</div>
      </header>
      <div className="app-body">
        ...
        <main className="content">
          ...
          {authenticated && <section className="workspace-grid">
            <article className="card order-card">
              <div className="section-heading"><div><span className="eyebrow">Customer / Order Core</span><h2>Create Draft Order</h2><p>Creates the customer if the phone is new, then atomically creates the order and items.</p></div><span className="check">Transactional</span></div>
              <form className="order-form" onSubmit={handleCreateOrder}>
                <div className="form-row"><label>Customer phone<input value={phone} onChange={(event) => setPhone(event.target.value)} onBlur={lookupCustomer} placeholder="05xxxxxxxx" required /></label><button className="secondary-button" type="button" onClick={lookupCustomer} disabled={!phone.trim()}>Find customer</button></div>
                {customerMessage && <p className="form-note">{customerMessage}</p>}
                <div className="form-row"><label>Customer name<input value={customerName} onChange={(event) => setCustomerName(event.target.value)} required /></label><label>City<input value={city} onChange={(event) => setCity(event.target.value)} /></label></div>
                ...
                {items.map((item, index) => <div className="item-row" key={index}><input aria-label={`Product description ${index + 1}`} placeholder="Product description" value={item.description} onChange={(event) => updateItem(index, 'description', event.target.value)} required /><input aria-label={`Quantity ${index + 1}`} type="number" min="1" step="1" value={item.quantity} onChange={(event) => updateItem(index, 'quantity', event.target.value)} required /><button className="remove-button" type="button" onClick={() => removeItem(index)} disabled={items.length === 1}>Remove</button></div>)}
                <div className="form-row"><label>Total Order Amount (AED)<input type="number" min="0" step="0.01" value={amount} onChange={(event) => setAmount(event.target.value)} required /></label><label>Notes<input value={notes} onChange={(event) => setNotes(event.target.value)} /></label></div>
                <button className="login-button" type="submit" disabled={orderLoading}>{orderLoading ? 'Creating order…' : 'Create Draft Order'}</button>
                {orderMessage && <p className={orderMessage.startsWith('Order ') ? 'form-success' : 'form-error'} role="status">{orderMessage}</p>}
              </form>
            </article>
          </section>}
