...
function MinimalAuthenticatedShell() {
  useEffect(() => {
    console.info('[AUTH-COMMIT-SUCCESS] minimal authenticated shell committed')
  }, [])
  return <div data-testid="minimal-auth-shell" data-auth-state="authenticated">Authenticated test shell</div>
}
...
function App() {
  ...
  const minimalShell = E2E && new URLSearchParams(window.location.search).get('e2eShell') === '1'
  ...
  if (minimalShell && authenticated) return <MinimalAuthenticatedShell />
