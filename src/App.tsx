...
import './App.css'
import { canAdministerUsers, getAuthConfig, hasOperationalAccess, signIn, signOut, type AuthState } from './lib/auth'
...
type AppProps = { auth: AuthState; onAuthChange: (auth: AuthState) => void }

function App({ auth, onAuthChange }: AppProps) {
  ...
  const environmentLabel = (import.meta.env.VITE_APP_ENVIRONMENT?.trim() || import.meta.env.MODE).toUpperCase()
  ...
  async function handleSignIn(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      setPassword('')
      window.history.replaceState({}, '', getPostLoginPath(window.location.search))
    } catch (signInError) {
  ...
  function navigateTo(item: string) {
    if (item === 'Reports') document.getElementById('reports-title')?.scrollIntoView({ behavior: 'smooth', block: 'start' })
  }
  ...
  async function lookupCustomer() {
    ...
    try {
      ...
      if (!customer) {
        setCustomerMessage('No existing customer found. A new customer will be created with the order.')
        return
      }
      ...
      setCustomerMessage(lookupError instanceof Error ? lookupError.message : 'Customer lookup failed')
    }
  }
