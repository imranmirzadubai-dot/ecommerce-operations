...
import { dispatchParcelsBulk, type BulkDispatchItem, type BulkDispatchResult } from '../lib/bulkDispatch'
...
export function BulkDispatchWorkspace({ accessToken }: Props) {
  ...
  async function handleScan(event: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      setQueue((current) => [...current, { p_parcel_id: found.id, p_tracking_id: found.tracking_id!.trim(), p_idempotency_key: crypto.randomUUID(), parcel_number: found.parcel_number, barcode: found.barcode, shipper_name: found.shippers!.name }])
      setScan('')
  ...
  async function handleDispatchBatch() {
    ...
    const batch = queue
    try {
      const batchResults = await dispatchParcelsBulk(accessToken, batch)
      setResults(batchResults)
      const successfulIds = new Set(batchResults.filter((result) => result.status === 'success').map((result) => result.item.p_parcel_id))
      setQueue((current) => current.filter((item) => !successfulIds.has(item.p_parcel_id)))
      ...
      setMessage(failureCount ? `${successCount} parcel${successCount === 1 ? '' : 's'} dispatched; ${failureCount} failed and remain queued for safe retry.` : `Bulk dispatch completed successfully for all ${successCount} parcels.`)
    } catch (dispatchError) {
  ...
  function removeParcel(parcelId: string) { if (dispatching) return; setQueue((current) => current.filter((item) => item.p_parcel_id !== parcelId)) }
  function clearQueue() { if (dispatching) return; setQueue([]); setResults([]); setMessage(''); setError(''); setScan(''); inputRef.current?.focus() }

  return (
    <section className="card bulk-dispatch-workspace" aria-labelledby="bulk-dispatch-title">
      <div className="section-heading"><div><span className="eyebrow">Dispatch Gate · T144</span><h2 id="bulk-dispatch-title">Bulk Dispatch</h2><p>Scan multiple Prepared parcels, build a batch, then review the outcome for every parcel.</p></div><span className="check">Operations / Admin</span></div>
      ...
      {results.length > 0 && <div className="bulk-results" aria-live="polite"><div className="bulk-queue-heading"><strong>Latest dispatch results</strong><span className="state-pill">{results.filter((result) => result.status === 'success').length} succeeded · {results.filter((result) => result.status === 'failure').length} failed</span></div><div className="orders-table-wrap"><table className="orders-table"><thead><tr><th>Parcel</th><th>Tracking ID</th><th>Result</th><th>Details</th></tr></thead><tbody>{results.map((result) => <tr key={result.item.p_parcel_id}><td><strong>{result.item.p_parcel_id}</strong></td><td>{result.item.p_tracking_id}</td><td><span className="state-pill">{result.status === 'success' ? 'Success' : 'Failure'}</span></td><td>{result.status === 'success' ? `Dispatched${result.result?.[0]?.dispatch_at ? ` at ${new Date(result.result[0].dispatch_at).toLocaleString()}` : ''}` : result.error}</td></tr>)}</tbody></table></div></div>}
