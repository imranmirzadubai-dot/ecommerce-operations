...
export function BulkDispatchWorkspace({ accessToken }: Props) {
  ...
  async function handleDispatchBatch() {
    ...
    try {
      ...
      setMessage('The batch stopped at the first unconfirmed dispatch. Retry only after resolving the reported command error.')
    } finally {
  ...
  return (
    <section className="card bulk-dispatch-workspace" aria-labelledby="bulk-dispatch-title">
      <div className="section-heading">
        <div><span className="eyebrow">Dispatch Gate · T143</span><h2 id="bulk-dispatch-title">Bulk Dispatch</h2><p>Scan multiple Prepared parcels, build a batch, then execute the same authoritative dispatch command once per parcel.</p></div>
        <span className="check">Operations / Admin</span>
      </div>
      <form className="dispatch-scan-form" onSubmit={handleScan}>
        <label htmlFor="bulk-dispatch-barcode">Parcel barcode</label>
        <div className="button-group">
          <input ref={inputRef} id="bulk-dispatch-barcode" value={scan} onChange={(event) => setScan(event.target.value)} placeholder="Scan parcel barcode" autoComplete="off" autoFocus inputMode="text" />
          <button className="login-button" type="submit" disabled={!scan.trim() || loading || dispatching}>{loading ? 'Adding…' : 'Add to batch'}</button>
        </div>
        <small className="form-note">Only Prepared parcels with an active assigned shipper and existing tracking ID are admitted.</small>
      </form>
      {error && <p className="form-error" role="alert">{error}</p>}
      {message && <p className="form-note" role="status">{message}</p>}
      {queue.length > 0 ? <>
        <div className="bulk-queue-heading"><strong>Dispatch batch</strong><span className="state-pill">{queue.length} parcel{queue.length === 1 ? '' : 's'}</span></div>
        <div className="orders-table-wrap">
          <table className="orders-table"><thead><tr><th>Parcel</th><th>Tracking ID</th><th>Shipper</th><th /></tr></thead><tbody>{queue.map((item) => <tr key={item.p_parcel_id}><td><strong>{item.parcel_number}</strong><small>{item.barcode}</small></td><td>{item.p_tracking_id}</td><td>{item.shipper_name}</td><td><button className="remove-button" type="button" onClick={() => removeParcel(item.p_parcel_id)} disabled={dispatching}>Remove</button></td></tr>)}</tbody></table>
        </div>
        <div className="button-group bulk-dispatch-actions"><button className="login-button" type="button" onClick={() => void handleDispatchBatch()} disabled={dispatching}>{dispatching ? 'Dispatching batch…' : `Dispatch ${queue.length} parcel${queue.length === 1 ? '' : 's'}`}</button><button className="secondary-button" type="button" onClick={clearQueue} disabled={dispatching}>Clear batch</button></div>
      </> : <p className="empty-state">No parcels in the dispatch batch. Scan a parcel to begin.</p>}
    </section>
  )
}
