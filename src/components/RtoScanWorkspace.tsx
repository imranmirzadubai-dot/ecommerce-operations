...
export function RtoScanWorkspace({ accessToken }: Props) {
  ...
  async function handleRto() {
    if (!parcel || processing || !['In Transit', 'NDR'].includes(parcel.state)) return
    setProcessing(true); setError(''); setMessage('')
    try {
      const result = await processRto(accessToken, { p_parcel_id: parcel.id, p_note: null, p_idempotency_key: crypto.randomUUID() })
      const processed = result[0]
      if (!processed) throw new Error('RTO completed without returning the parcel')
      setParcel({ ...parcel, state: processed.state })
      setScan('')
      setMessage(`Parcel ${processed.parcel_number} processed as RTO. Stored shipper: ${processed.shipper_name ?? 'not assigned'}.`)
    } catch (rtoError) {
      setError(rtoError instanceof Error ? rtoError.message : 'RTO processing failed')
    } finally {
      setProcessing(false); window.setTimeout(() => inputRef.current?.focus(), 0)
    }
  }

  const canProcess = parcel !== null && ['In Transit', 'NDR'].includes(parcel.state)
  ...
  return <>
    <section className="card dispatch-scan-workspace" aria-labelledby="rto-scan-title">
      ...
      {parcel && <div className="button-group dispatch-actions"><button className="login-button" type="button" onClick={() => void handleRto()} disabled={!canProcess || processing}>{processing ? 'Processing RTO…' : 'Process RTO'}</button><button className="secondary-button" type="button" onClick={() => { setScan(''); setParcel(null); setMessage(''); setError(''); inputRef.current?.focus() }} disabled={loading || processing}>Scan another parcel</button></div>}
    </section>
