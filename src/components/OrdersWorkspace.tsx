...
function downloadOrdersCsv(orders: OrderListRow[]) {
  ...
  const rows = orders.map((order) => [order.order_number, order.customers?.name ?? '', order.customers?.phone ?? '', order.lifecycle_state, Number(order.original_amount).toFixed(2), order.order_date, order.order_items.map((item) => `${item.description} x${item.quantity}`).join('; ')])
...
export function OrdersWorkspace({ accessToken }: Props) {
  ...
  return (
    <section className="card orders-workspace" aria-labelledby="orders-title">
      ...
      {detailOrder && <div className="order-editor" role="dialog" aria-modal="true" aria-labelledby="order-detail-title"><div className="section-heading"><div><span className="eyebrow">Order Detail</span><h3 id="order-detail-title">{detailOrder.order_number}</h3><p>Full detail for the selected order.</p></div><button className="secondary-button" type="button" onClick={() => setDetailOrder(null)}>Back to Orders</button></div><div className="order-form"><div className="form-row"><div><strong>Customer</strong><p>{detailOrder.customers?.name ?? '—'}</p></div><div><strong>Phone</strong><p>{detailOrder.customers?.phone ?? '—'}</p></div></div><div className="form-row"><div><strong>City</strong><p>{detailOrder.customers?.city ?? '—'}</p></div><div><strong>Order date</strong><p>{detailOrder.order_date}</p></div></div><div><strong>Address</strong><p>{detailOrder.customers?.address ?? '—'}</p></div><div className="form-row"><div><strong>Lifecycle state</strong><p><span className="state-pill">{detailOrder.lifecycle_state}</span></p></div><div><strong>Amount</strong><p>AED {Number(detailOrder.original_amount).toFixed(2)}</p></div></div><div><strong>Order items</strong><ul>{detailOrder.order_items.map((item, index) => <li key={`${item.description}-${index}`}>{item.description} × {item.quantity}</li>)}</ul></div>{detailOrder.notes && <div><strong>Notes</strong><p>{detailOrder.notes}</p></div>}<div className="button-group"><button className="secondary-button" type="button" onClick={() => { setDetailOrder(null); void openTimeline(detailOrder) }}>Open Timeline</button>{detailOrder.lifecycle_state === 'Draft' && <button className="secondary-button" type="button" onClick={() => { setDetailOrder(null); startEditing(detailOrder) }}>Edit Draft</button>}</div></div></div>}
