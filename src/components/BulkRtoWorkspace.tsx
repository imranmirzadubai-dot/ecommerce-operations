...
type Props = { accessToken: string; resolveParcelByBarcode: (accessToken: string, barcode: string) => Promise<Parcel | null> }
...
export function BulkRtoWorkspace({ accessToken, resolveParcelByBarcode }: Props) {
  ...
  async function resolveBatch() {
    ...
    try {
      const resolved: Parcel[] = []; const failures: string[] = []
      for (const barcode of barcodes) {
        try { const parcel = await resolveParcelByBarcode(accessToken, barcode); if (!parcel) failures.push(`${barcode}: unknown barcode`); else resolved.push(parcel) }
        catch (lookupError) { failures.push(`${barcode}: ${lookupError instanceof Error ? lookupError.message : 'lookup failed'}`) }
      }
      setItems(resolved); if (failures.length) setError(failures.join(' · '))
  ...
  async function processBatch() {
    ...
    try {
      const batchResults = await processRtoBulk(accessToken, eligible.map((parcel) => ({ p_parcel_id: parcel.id, p_note: null, p_idempotency_key: `rto:${parcel.id}` })))
      setResults(batchResults)
      const updated = new Map(batchResults.flatMap((entry) => (entry.result ?? []) as ProcessRtoResult[]).map((entry) => [entry.parcel_id, entry]))
      setItems((current) => current.map((parcel) => { const result = updated.get(parcel.id); return result ? { ...parcel, state: result.state } : parcel }))
    } catch (batchError) { setError(batchError instanceof Error ? batchError.message : 'Bulk RTO processing failed') }
  ...
  return <section className="card dispatch-scan-workspace" aria-labelledby="bulk-rto-title">
    ...
    <label htmlFor="bulk-rto-barcodes">Parcel barcodes</label><textarea id="bulk-rto-barcodes" value={input} onChange={(event) => setInput(event.target.value)} rows={5} placeholder="One barcode per line, or paste a comma/space-separated batch" disabled={resolving || processing} />
