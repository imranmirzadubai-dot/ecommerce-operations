...
export function DispatchScanWorkspace({ accessToken }: Props) {
  ...
  function resetScan() {
    ...
    setScan(''); setParcel(null); setTracking(''); setMessage(''); setError('')
  ...
  return (
    <section className="card dispatch-scan-workspace" aria-labelledby="dispatch-scan-title">
      ...
      <form className="dispatch-scan-form" onSubmit={handleScan}>
        <label htmlFor="dispatch-barcode">Parcel barcode</label>
        <div className="button-group">
          <input ref={inputRef} id="dispatch-barcode" value={scan} onChange={(event) => setScan(event.target.value)} placeholder="Scan parcel barcode" autoComplete="off" autoFocus inputMode="text" aria-describedby="dispatch-scan-help" />
          <button className="login-button" type="submit" disabled={!scan.trim() || loading || dispatching}>{loading ? 'Resolving…' : 'Resolve parcel'}</button>
        </div>
        <small id="dispatch-scan-help" className="form-note">The barcode is the immutable parcel number. Scanner input should submit with Enter.</small>
      </form>
      {error && <p className="form-error" role="alert">{error}</p>}
      {message && <p className={parcel?.state === 'Dispatched' ? 'form-success' : 'form-note'} role="status">{message}</p>}
      {parcel && <div className="dispatch-parcel-panel">
        ...
        <div><span className="eyebrow">Assigned shipper</span><strong>{parcel.shippers?.name ?? 'Not assigned'}</strong><small>{parcel.shippers?.active ? 'Active' : 'Inactive / unavailable'}</small></div>
        <label><span className="eyebrow">Tracking ID</span><input value={tracking} onChange={(event) => setTracking(event.target.value)} placeholder="Enter tracking ID" disabled={dispatching || parcel.state !== 'Prepared'} /></label>
      </div>}
